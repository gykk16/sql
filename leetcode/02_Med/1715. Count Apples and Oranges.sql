...
SELECT *
  FROM (SELECT B.*
             ...
             , B.APPLE_COUNT + NVL(C.APPLE_COUNT, 0)
             , B.ORANGE_COUNT + NVL(C.ORANGE_COUNT, 0)
          FROM BOXES B
             , CHESTS C
         WHERE B.CHEST_ID = C.CHEST_ID(+)) TAB_A;

SELECT SUM(B.APPLE_COUNT + NVL(C.APPLE_COUNT, 0))   AS APPLE_COUNT
     , SUM(B.ORANGE_COUNT + NVL(C.ORANGE_COUNT, 0)) AS ORANGE_COUNT
  FROM BOXES B
     , CHESTS C
