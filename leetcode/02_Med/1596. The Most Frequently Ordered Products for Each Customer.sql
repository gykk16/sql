...
SELECT *
  FROM (SELECT TAB_A.CUSTOMER_ID
              ,CASE
                   WHEN MAX(TAB_A.FREC) OVER(PARTITION BY TAB_A.CUSTOMER_ID) = TAB_A.FREC THEN
                    TAB_A.PRODUCT_ID
                   ELSE
                    NULL
               END AS PRODUCT_ID
          ...
          FROM (SELECT CUSTOMER_ID
                 ...
                 GROUP BY CUSTOMER_ID
                         ,PRODUCT_ID) TAB_A
              ,PRODUCTS P
         WHERE TAB_A.PRODUCT_ID = P.PRODUCT_ID
         ORDER BY CUSTOMER_ID
                 ,PRODUCT_ID) TAB_B
 WHERE TAB_B.PRODUCT_ID IS NOT NULL;
 
 
SELECT *
  FROM (SELECT TAB_A.CUSTOMER_ID
              ,DECODE((MAX(TAB_A.FREC) OVER(PARTITION BY TAB_A.CUSTOMER_ID)),
                      TAB_A.FREC,
                      TAB_A.PRODUCT_ID,
                      NULL) AS PRODUCT_ID
              ,P.PRODUCT_NAME
