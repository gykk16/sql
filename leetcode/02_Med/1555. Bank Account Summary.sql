...
SELECT U.USER_ID
     , U.USER_NAME
     , U.CREDIT + NVL(TAB_B.SUM_, 0)                                AS CREDIT
     , CASE WHEN U.CREDIT + TAB_B.SUM_ < 0 THEN 'Yes' ELSE 'No' END AS CREDIT_LIMIT_BREACHED
  FROM USERS U
     , (SELECT USER_ID, SUM(AMOUNT) AS SUM_
          FROM (SELECT PAID_BY AS USER_ID, AMOUNT * -1 AS AMOUNT
                  FROM TRANSACTIONS
                 UNION ALL
                SELECT PAID_TO AS USER_ID, AMOUNT
                  FROM TRANSACTIONS) TAB_A
         GROUP BY USER_ID) TAB_B
 WHERE U.USER_ID = TAB_B.USER_ID(+)
 ORDER BY U.USER_ID;
