...
SELECT GENDER, TO_CHAR(DAY, 'YYYY-MM-DD') AS DAY, TOTAL
  FROM (
           SELECT S2.GENDER
                , S2.DAY
                , (SELECT SUM(S.SCORE_POINTS)
                     FROM SCORES S
                    WHERE S.GENDER = 'F' AND S.DAY <= S2.DAY) AS TOTAL
             FROM SCORES S2
            WHERE S2.GENDER = 'F'
           ...
           SELECT S2.GENDER
                ...
                , (SELECT SUM(S.SCORE_POINTS)
                     FROM SCORES S
                    WHERE S.GENDER = 'M' AND S.DAY <= S2.DAY) AS TOTAL
             FROM SCORES S2
            WHERE S2.GENDER = 'M'
            GROUP BY S2.DAY, S2.GENDER) TAB_A
 ORDER BY TAB_A.GENDER, TAB_A.DAY
;



SELECT S2.GENDER
     , TO_CHAR(S2.DAY, 'YYYY-MM-DD')                     AS DAY
     , (SELECT SUM(S.SCORE_POINTS)
          FROM SCORES S
         WHERE S.GENDER = S2.GENDER AND S.DAY <= S2.DAY) AS TOTAL
  FROM SCORES S2
 GROUP BY S2.GENDER, S2.DAY
 ORDER BY S2.GENDER, DAY
;
