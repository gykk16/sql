...
SELECT T2.ACCOUNT_ID
     , TO_CHAR(T2.DAY, 'YYYY-MM-DD') AS "DAY"
     , (SELECT SUM(CASE
                       WHEN UPPER(T1.TYPE) = UPPER('DEPOSIT') THEN T1.AMOUNT
                       ELSE T1.AMOUNT * -1
                   END)
          FROM TRANSACTIONS T1
         WHERE T1.ACCOUNT_ID = T2.ACCOUNT_ID AND T1.DAY <= T2.DAY) AS BALANCE
  FROM TRANSACTIONS T2
 ORDER BY T2.ACCOUNT_ID, T2.DAY;