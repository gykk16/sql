...
SELECT MIN(EVENT_DATE) AS INSTALL_DT
     , PLAYER_ID
  FROM ACTIVITY
 GROUP BY PLAYER_ID;

SELECT COUNT(DISTINCT INSTALL_DT)
...
--AND A.EVENT_DATE > TAB_A.INSTALL_DT
;
  ...
  FROM (SELECT TAB_A.PLAYER_ID
          ...
          FROM ACTIVITY A
             , (SELECT PLAYER_ID, MIN(EVENT_DATE) AS INSTALL_DT
                  FROM ACTIVITY
                 GROUP BY PLAYER_ID) TAB_A
         WHERE A.EVENT_DATE = TAB_A.INSTALL_DT + 1
           AND A.PLAYER_ID = TAB_A.PLAYER_ID
       ) TAB_B
 GROUP BY INSTALL_DT;
